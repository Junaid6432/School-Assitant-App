...
export default function BulkPreviewPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col">
      {/* Sticky Control Bar */}
      <div className="sticky top-0 z-[100] bg-slate-100/80 backdrop-blur-md px-4 md:px-8 py-6 print:hidden">
        <div className="max-w-[1200px] mx-auto bg-white p-6 rounded-[2rem] shadow-xl border border-slate-200/50 flex flex-col gap-6">
          <div className="flex flex-wrap items-center justify-between gap-6">
            <div className="flex items-center gap-6">
              <button 
                onClick={() => router.back()}
                className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-slate-100 hover:text-slate-900 transition-all shadow-inner"
              >
                <ChevronLeft className="w-6 h-6" />
              </button>
              <div className="space-y-1">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none">Bulk Preview</h3>
                <p className="text-xl font-black text-slate-900 leading-none">{teacher?.assignedClass}</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-2xl border border-slate-100">
                <span className="text-[10px] font-black text-slate-700 uppercase px-3">Style</span>
                <select 
                  value={selectedStyle}
                  onChange={(e) => setSelectedStyle(e.target.value as DMCStyle)}
                  className="p-2.5 rounded-xl border-2 border-slate-200 bg-white text-slate-950 text-[11px] font-black uppercase outline-none focus:ring-2 focus:ring-primary/20 shadow-sm transition-all min-w-[160px]"
                >
                  {STYLES.map(s => (
              ...
              <button 
                onClick={handlePrint}
                className="flex items-center gap-3 bg-slate-900 text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-800 transition-all shadow-[0_15px_30px_rgba(15,23,42,0.3)] active:scale-95 border-b-4 border-slate-950"
              >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-50">
            <div className="relative w-full sm:w-80">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl border-2 border-slate-600 bg-white shadow-sm focus:ring-2 focus:ring-primary/20 outline-none transition-all text-xs font-bold placeholder:text-slate-500"
                value={searchTerm}
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 bg-slate-100 px-4 py-2 rounded-full">
                ...
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                  {filteredResults.length} Records Loaded
                </span>
              </div>
              <select 
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value)}
                className="bg-transparent border-none outline-none text-[10px] font-black uppercase tracking-widest text-slate-400 cursor-pointer"
              >
      ...
      {/* Preview Area */}
      <div className="flex-1 p-4 md:p-8 lg:p-12 print:p-0 flex justify-center preview-container">
        <div className="w-full max-w-[900px] flex flex-col items-center space-y-12 print:space-y-0 print:max-w-none print:w-full print:block bulk-print-wrapper">
